...
class EjemploGSON
{
  ...
  public static void main(String[] args)
  {
    ...
    Gson j = new GsonBuilder().setDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS").create(); //SetFormat para utilizar un formato de fecha (ISO8601)

    String s = j.toJson(e); //Pasa el arreglo de objetos a JSON o serializar
    System.out.println(s);

    Empleado[] v = (Empleado[])j.fromJson(s,Empleado[].class); //Para des-serializar y cast a Empleado.

    for (int i = 0; i < v.length; i++)
      System.out.println(v[i].nombre + " " + v[i].edad + " " + v[i].sueldo + " " + v[i].fecha_ingreso + " jefe:"
      + (v[i].jefe != null ? v[i].jefe.nombre : "nadie" ) );
  }
}