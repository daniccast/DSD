...
public class Servicio
{
  ...
  @POST
  @Path("alta")//Nombre del método web
  @Consumes(MediaType.APPLICATION_FORM_URLENCODED) //Los parametros del metodo se reciben codificados como URL
  @Produces(MediaType.APPLICATION_JSON) //Regresar resultados en JSON
  public Response alta_usuario(@FormParam("usuario") Usuario usuario) throws Exception
  { //FormParam de el nombre del parametro web, puede er diferente al del programa
    ...
    {
      conexion.close(); //IMportante cerrar
    }
...
@POST
  ...
  public Response borrar_usuarios() throws Exception
  {
    ...
    try{
      PreparedStatement stmt_1 = conexion.prepareStatement("SELECT count(*) as cu FROM usuarios");
      try{
